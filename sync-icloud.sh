...
send_notification()
{
   ...
   unique_file_count=$(find "$download_path" -type f ! -path '*/.*' -printf '%P\n' | grep -v '/\.' | sed -E 's/_HEVC\.MOV$|(\.[^.\/]+)$//' | sort -u | wc -l)
   # 追加数量到通知消息中
   notification_message="${notification_message}（${unique_file_count}）"
...
command_line_builder()
{
   ...
   if [ "${dry_run}" != false ]
   then
      command_line="${command_line} --dry-run"
   fi
   if [ "${skip_live_photos}" = false ]
   then
