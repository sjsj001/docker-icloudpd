...
ARG icloudpd_version="0.0.1234567890"
...
COPY icloudpd-0.0.1234567890-py3-none-any.whl /opt/
...
echo "$(date '+%d/%m/%Y - %H:%M:%S') | Install iCloudPD latest release" && \
   ...
   pip3 install --no-cache-dir /opt/icloudpd-0.0.1234567890-py3-none-any.whl && \
   deactivate && \
   apk del build
